...
def eval_det_cls(pred, gt, iou_thr=None):
    ...
    for img_id in pred.keys():
        ...
        pred_centerness = []
        box_idx = 0
        for box, score, centerness in pred[img_id]:
            image_ids.append(img_id)
            confidence.append(score)
            pred_cur[box_idx] = box.tensor
            pred_centerness.append(centerness)
            box_idx += 1
...
def indoor_eval(gt_annos,
    ...
    for img_id in range(len(dt_annos)):
        ...
        for i in range(len(det_anno['labels_3d'])):
            ...
            centerness = det_anno['centerness'].numpy()[i]
            ...
            if img_id not in gt[label]:
                gt[int(label)][img_id] = []
            pred[int(label)][img_id].append((bbox, score, centerness))
