...
class SingleStageSparse3DDetector(Base3DDetector):
    ...
    def forward_train(self,
        ...
        losses = self.neck_with_head.loss(
    ...
    def simple_test(self, points, img_metas, imgs=None, rescale=False, **kwargs):
        ...
        x = self.extract_feat(points, img_metas)
        bbox_list = self.neck_with_head.get_bboxes(
...
class SingleStageSparse3DDetector_CA(Base3DDetector):
    ...
    def forward_train(self,
        ...
        losses, targets = self.neck_with_head.loss(
            *x, gt_bboxes_3d, gt_labels_3d, img_metas)
        # img branch
        if self.with_img_backbone:
        ...
        stage2_targets = self.get_stage2_targets(targets, select_points[2])
        stage2_losses = self.stage2_head.loss(
            stage2_preds, stage2_targets, select_points[0])
    ...
    def simple_test(self, points, img_metas, img=None, rescale=False, **kwargs):
        """Test function without augmentaiton."""
        x, select_points = self.extract_feat(points, img_metas)
        stage1_results = self.neck_with_head.get_bboxes(
            *x, img_metas, rescale=rescale)
        # img branch
        if self.with_img_backbone:
            batch_input_shape = tuple(img[0].size()[-2:])
            ...
            img_dict = None
        stage2_preds = self.stage2_head(select_points, img_dict)
        stage2_results = self.stage2_head.get_bboxes(
            stage2_preds, select_points[0], img_metas, rescale=rescale)
        bbox_list = []
        # per image
        for i in range(len(stage2_results)):
            results = [stage1_results[i], stage2_results[i]]
            ensemble_bboxes, ensemble_scores, ensemble_centernesses = [], [], []
            for stage in self.test_cfg.ensemble_stages:
                ensemble_bboxes.append(results[stage][0])
                ensemble_scores.append(results[stage][1])
                ensemble_centernesses.append(results[stage][2])
            ensemble_bboxes = torch.cat(ensemble_bboxes, dim=0)
            ensemble_scores = torch.cat(ensemble_scores, dim=0)
            ensemble_centernesses = torch.cat(ensemble_centernesses, dim=0)
            nms_process = self.neck_with_head._nms(
                ensemble_bboxes, ensemble_scores, ensemble_centernesses, img_metas[i])
            bbox_list.append(nms_process)
        bbox_results = [
            bbox3d2result_with_centerness(bboxes, scores, labels, centerness)
            for bboxes, scores, labels, centerness in bbox_list
        ]
        return bbox_results
...
class TwoStageSparse3DDetector(SingleStageSparse3DDetector_CA):
    def simple_test(self, points, img_metas, img=None, rescale=False, **kwargs):
        ...
        if self.with_img_backbone:
            ...
            img_dict = None
        stage2_preds = self.stage2_head(select_points, img_dict)
        stage2_results_all = self.stage2_head.get_bboxes(
            stage2_preds, select_points[0], img_metas, rescale=rescale)
...
def bbox3d2result_with_centerness(bboxes, scores, labels, centerness, attrs=None):
    result_dict = dict(
...
@DETECTORS.register_module()
class CascadeSparse3DDetector(TwoStageSparse3DDetector):

    def extract_feat(self, points, gt_bboxes_3d):
        """Extract features from points."""
        coordinates, features = ME.utils.batch_sparse_collate(
            [(p[:, :3] / self.voxel_size, p[:, 3:] / 255.) for p in points],
            device=points[0].device)
        ...
        # IPython.embed()
        # exit()
        x, select_points = self.neck_with_head(x, gt_bboxes_3d)
        return x, select_points
    ...
    def forward_train(self,
                      ...
                      img_metas,
                      img=None):
        self.neck_with_head.epoch = self.epoch
        # self.neck_with_head.epoch = self.epoch
        x, select_points = self.extract_feat(points, gt_bboxes_3d)
        losses, targets = self.neck_with_head.loss(
            *x, gt_bboxes_3d, gt_labels_3d, img_metas)
        # img branch
        if self.with_img_backbone:
            ...
            img_dict = None
        stage2_preds = self.stage2_head(select_points, img_dict)
        stage2_losses = self.stage2_head.loss_cascade(
            stage2_preds, select_points[0], gt_bboxes_3d, gt_labels_3d,
            img_metas, self.neck_with_head.assigner, select_points)
        losses.update(stage2_losses)
        return losses

@DETECTORS.register_module()
class PointCascadeSparse3DDetector(CascadeSparse3DDetector):
    ...
    def forward_train(self,
        ...
        losses, targets = self.neck_with_head.loss(
            *x, gt_bboxes_3d, gt_labels_3d, img_metas)
        # img branch
        stage2_preds = self.stage2_head(select_points, None)
        stage2_losses = self.stage2_head.loss_cascade(
            stage2_preds, select_points[0], gt_bboxes_3d, gt_labels_3d,
            img_metas, self.neck_with_head.assigner, select_points)
        losses.update(stage2_losses)
